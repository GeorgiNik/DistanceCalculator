using System;
using DistanceCalculator.Application.Common.Interfaces;
using DistanceCalculator.Domain.Entities;
using DistanceCalculator.Infrastructure.Persistence;
using IdentityServer4.EntityFramework.Options;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Options;
using Moq;

namespace DistanceCalculator.Application.UnitTests
{
    public static class ApplicationDbContextFactory
    {
        public static DistanceCalculatorDbContext Create()
        {
            var options = new DbContextOptionsBuilder<DistanceCalculatorDbContext>()
                .UseInMemoryDatabase(Guid.NewGuid().ToString())
                .Options;

            var operationalStoreOptions = Options.Create(
                new OperationalStoreOptions
                {
                    DeviceFlowCodes = new TableConfiguration("DeviceCodes"),
                    PersistedGrants = new TableConfiguration("PersistedGrants")
                });

            var dateTimeMock = new Mock<IDateTime>();
            dateTimeMock.Setup(m => m.Now)
                .Returns(new DateTime(3001, 1, 1));

            var currentUserServiceMock = new Mock<ICurrentUser>();
            currentUserServiceMock.Setup(m => m.UserId)
                .Returns("00000000-0000-0000-0000-000000000000");

            var context = new DistanceCalculatorDbContext(
                options, operationalStoreOptions,
                currentUserServiceMock.Object, dateTimeMock.Object);

            context.Database.EnsureCreated();


            return context;
        }

        public static void Destroy(DistanceCalculatorDbContext context)
        {
            context.Database.EnsureDeleted();
            context.Dispose();
        }
    }
}